...
func (this *MainController)ShowIndex()  {
	...
	var articles []models.Article  //查询所有的属性
	...
	pageCount1 := math.Ceil(pageCount)  //向上取正数  math.Ceil     向下取整 math.floor
	...
	LastPage:= false
	Lastpagecont := int(pageCount1)
	if pageIndex1 == Lastpagecont{
	...
	this.Data["articleType"] = artiTypes
...
//显示添加文章界面
func (this*MainController)ShowAdd(){
	// 1.获取数据
	o:= orm.NewOrm()
...
func (c*MainController)HandleAdd(){
	...
	article.Aimg =img
	//article.Acount = 11
	beego.Info(artiName,artiContent,img)
	...
	if typeName ==""{
		...
		//beego.Info("arti.ArticleType is ",&artiType)
